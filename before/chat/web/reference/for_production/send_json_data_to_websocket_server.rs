...
pub struct State {
    ...
    socket_responses: Vec<Option<String>>, // is this right approach?
    value: String,
}
...
impl Component for Model {
    ...
    fn create(_: Self::Properties, link: ComponentLink<Self>) -> Self {
        let state = State {
            user_id: None,
            user_inputs: Vec::new(),
            socket_responses: vec![None], // is this right approach?
            value: "".into(),
        };
        Model {
    ...
    fn update(&mut self, msg: Self::Message) -> ShouldRender {
        match msg {
            ...
            Msg::WebSocketReady(response) => { // payload?
                ...
                self.state.socket_responses.push(socket_input);
            }
            Msg::Ignore => {
                return false;
            }
...
impl Model {
    fn ws_data(&self) -> Html<Model> {
        if self.state.socket_responses == vec![None] { // is it right?
            html! {
                <li>{ "There is no messages yet." }</li>
            } 
        } else {
            // should show all messages with <li>, then work in other socket connection also
            html! {
                <>
                    <li>{ format!("You will send {:#?} in JSON", &self.state.value) }</li>
                </>
            }
        }
    }
