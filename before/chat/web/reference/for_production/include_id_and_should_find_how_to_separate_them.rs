...
pub struct State {
    ...
    ws_responses: Vec<Option<String>>, // is this right approach? or Vec<String>, it is necessary to make macro work
    value: String,
}
        ...
        html! {
            <section>
                <nav id="nav", class=("nav", "flex", "center"), >
                    ...
                    // It will be better to separate enter and exit in different pages.
                    <button
                        id="connect",
...
fn view_response((_idx, response): (usize, &Option<String>)) -> Html<Model> {
    ...
    if let Some(value) = response {
        ...
        let WebSocketResponse { value, client, number_of_connection } = ws_response;

        // equals to use tuple and manually destructure
        // let desctruct = (ws_response.value, ws_response.client, ws_response.number_of_connection);
        // let (value, client, number_of_connection) = desctruct;
        ...
        if let Some(user_id) = client {
            ...
            if user_id == self.state.user_id {
                ...
                html! {
                    <li>
                        <span>{ format!("{}: {}", user_id, value) }</span>
                    </li>
                }
            }
