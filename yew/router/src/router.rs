...
pub struct Route<T> {
    pub pathname: Vec<String>, // use this to be compatible with self.location.pathname().unwrap();
    ...
    where T: JsSerialize + Clone + TryFrom<Value> + Default +'static
{
    ...
    pub fn current_route(route_service: &RouteService<T>) -> Self
    {
        ...
        let mut pathname: Vec<String> = path.split("/").map(String::from).collect();
        pathname.remove(0); // remove empty string that is split from the first '/'
