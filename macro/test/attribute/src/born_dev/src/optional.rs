        ...
        nested_macro! {
            ($s:tt) => {
                macro_rules! $basestruct {
                    ($s($pub:tt)?) => {
                        $s($pub)? struct $basestruct {
                            $( $commonfield: $commonty, )*
                        }
                    };
                    (#[derive($s($arg:tt)+)] $s($pub:tt)?) => {
                        #[derive($s($arg)+)]
                        $s($pub)? struct $basestruct {
                            $( $commonfield: $commonty, )*
                        }
                    };
                    ($s($pub:tt)? struct $name:ident { $s( $field:ident: $ty:ty ),* $s(,)* }) => {
                        $s($pub)? struct $name {
                            $( $commonfield: $commonty, )*
                            $s( $field: $ty ),*
                        }
                    };
                    (#[derive($s($arg:tt)+)] $s($pub:tt)? struct $name:ident { $s( $field:ident: $ty:ty ),* $s(,)* }) => {
                        #[derive($s($arg)+)]
                        $s($pub)? struct $name {
                            $( $commonfield: $commonty, )*
                            $s( $field: $ty ),*
                    ...
                    (reuse $s($pub:tt)? struct $name:ident ) => {
                        $s($pub)? struct $name {
                            $( $commonfield: $commonty, )*
                        }
                    };
                    (#[derive($s($arg:tt)+)] reuse $s($pub:tt)? struct $newname:ident ) => {
                        #[derive($s($arg)+)]
                        $s($pub)? struct $newname {
                            $( $commonfield: $commonty, )*
                        }
                    };
                }
